...
public interface PatientMapper {
	...
	@Select("<script>"
			...
			+ "patient_no like CONCAT('%',#{readPatientListDto.patientNoKey},'%') "
			+ "</if> "
			+ "<if test='readPatientListDto.patientNameKey != null'> "
			+ "patient_name like CONCAT('%',#{readPatientListDto.patientNameKey},'%') "
			+ "</if> "	
			+ "<if test='readPatientListDto.patientMaleKey != null'> "
			+ "patient_male like CONCAT('%',#{readPatientListDto.patientMaleKey},'%') "
			+ "</if> "
			+ "<if test='readPatientListDto.patientPhoneKey != null'> "
			+ "patient_phone like CONCAT('%',#{readPatientListDto.patientPhoneKey},'%') "
			+ "</if> "
			+ "<if test='readPatientListDto.patientRrnKey != null'> "
			+ "patient_rrn like CONCAT('%',#{readPatientListDto.patientRrnKey},'%') "
			+ "</if> "
			...
			+ "patient_birth &lt;= #{readPatientListDto.patientBirthEnd} "
			+ "</if> "
			+ "</where> "
