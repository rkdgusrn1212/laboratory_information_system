...
export default function ReceptCollection() {
    ...
    const selectpatient = (i) => {
        ...
        setNawon({
            visits: [
                { visitno: 1, v_doc: "김의사", department_name: "소화기내과(GI) - 제2병동", visit_date: "22-11-15" },
                { visitno: 2, v_doc: "김의사", department_name: "소화기내과(GI) - 제2병동", visit_date: "22-12-16" },
                { visitno: 3, v_doc: "나의사", department_name: "신경외과(NS) - 제1병동", visit_date: "23-01-5" },
                { visitno: 4, v_doc: "나의사", department_name: "신경외과(NS) - 제1병동", visit_date: "23-01-18" },
                { visitno: 5, v_doc: "김의사", department_name: "소화기내과(GI) - 제2병동", visit_date: "23-01-27" }
            ]
        });
        // 선택한 환자로 처방 받아오기
        ...
        selectionModel1.map((id, i) => {
            ...
            rows.map((row, i) => {
                if (row.id === id) {
                    ...
                    rows2.push({
                        p_code: row.p_code,
                        id: row.id, visit_no: row.visit_no, order_date: row.order_date,
                        test_container: row.test_container, staff_name: row.staff_name,
                        department_name: row.department_name, test_name: row.test_name
                    })
                }
            })
        })
        ...
        selectionModel2.map((id, i) => {
            ...
            rows.map((row, i) => {
                if (row.id === id) {
                    ...
                    rows4.push({
                        id: row.id, visit_no: row.visit_no, order_date: row.order_date,
                        specimen_no: row.specimen_no, test_container: row.test_container, staff_name: row.staff_name,
                        department_name: row.department_name, test_name: row.test_name
                    })
                }
            })
                ...
                <Card sx={{ minWidth: 275, width: '95%' }}>
                    <CardContent>
                        <Box
                            ...
                            autoComplete="off"
                        >
                            <Grid sx={{ maxHeight: '500px', overflowY: 'scroll' }}><h3>&nbsp; 환자 정보 검색창</h3>
                                ...
                                {callpatient.patients && callpatient.patients.map((patient, i) => {
                                    if (!callpatient.patients) { return <Grid>no data</Grid>; }
                                    else if (selectedp === (i + 1))
                                        return <Grid sx={{
                                            textAlign: 'center', display: 'flex', justifyContent: 'center', mx: 3,
            ...
            <Grid sx={{ minWidth: ' 1180px' }}>
                <Grid sx={{ width: '25%', float: 'left', mx: 1 }} >
                    <Card sx={{ height: '700px', minWidth: 275, width: '95%', mx: 1, overflowY: 'scroll' }}>
                        <CardContent>
                            <h3>내원정보</h3>
                            {nawon.visits && nawon.visits.map((visit, i) => {
                                if (!nawon.visits) {
                ...
                {
                    ordern.order && ordern.order.map((id, i) => {
                        if (!ordern.order) {
                            return <Grid>no data</Grid>;
                        }
                        else {
                            return <Card sx={{ minWidth: '869px', width: '73%', marginleft: 20 }}>
                                <CardContent sx={{ minWidth: 500 }}>
                                    ...
                                    <Box sx={{
                                        ...
                                        <br />
                                        <Button sx={{ width: '100%' }} variant="outlined" onClick={grid1buttonclick}>
                                            리스트 등록
                                        </Button>
                                    </Box>
